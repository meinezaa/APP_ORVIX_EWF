...
class _NotifikasiViewState extends State<NotifikasiView> {
  ...
  final List<NotifikasiModel> _notifications = [
    NotifikasiModel(
      id: 'calculation-saved',
      title: 'Perhitungan tersimpan',
      message: 'Hasil perhitungan Emas Fisik berhasil disimpan.',
      time: '08.30',
    ...
    NotifikasiModel(
      id: 'gold-price-updated',
      title: 'Harga emas diperbarui',
      message: 'Data harga emas terbaru sudah tersedia.',
      time: '07.45',
    ...
    NotifikasiModel(
      id: 'welcome-orvix',
      title: 'Selamat datang di ORVIX',
      message: 'Mulai gunakan kalkulator untuk membantu perhitungan Anda.',
      time: 'Kemarin',
    ...
    NotifikasiModel(
      id: 'saved-pivot-example',
      title: 'Perhitungan tersimpan',
      message: 'Hasil perhitungan Pivot Point berhasil disimpan.',
      time: 'Kemarin',
      icon: Icons.check_circle_outline,
      isRead: true,
      group: 'Kemarin',
    ...
    NotifikasiModel(
      id: 'history-reminder',
      title: 'Jangan lupa cek histori',
      message: 'Lihat kembali seluruh aktivitas perhitungan Anda.',
      time: 'Kemarin',
      icon: Icons.history,
      isRead: true,
      group: 'Kemarin',
    ...
    return StreamBuilder<List<HistoryModel>>(
      ...
      builder: (context, historySnapshot) => StreamBuilder<Set<String>>(
        ...
        builder: (context, statusSnapshot) {
          ...
          final notifications = [
            ..._notifications,
            ..._buildPivotNotifications(
              historySnapshot.data ?? const <HistoryModel>[],
              readIds,
            ),
          ];
          final visible = notifications
              .where(
                (item) =>
                    _selectedFilter == 0 ||
                    (_selectedFilter == 1 ? item.isRead : !item.isRead),
              )
              .toList();
          return Scaffold(
  ...
  List<NotifikasiModel> _buildPivotNotifications(
    ...
    Set<String> readIds,
      ...
      .map((item) {
        ...
        return NotifikasiModel(
          id: id,
          title: 'Hasil Pivot Point: $indication',
          message:
              'Nilai Pivot Point Anda adalah ${item.hasil.toStringAsFixed(2)}. Indikasi: $indication.',
          time: _formatTime(item.createdAt),
          icon: indication == 'BUY' ? Icons.trending_up : Icons.trending_down,
          isRead: readIds.contains(id),
          group: _isToday(item.createdAt) ? 'Hari ini' : 'Kemarin',
        );
      })
      .toList();
  ...
  Widget _buildFilter() => Container(
    ...
    child: Row(
      children: [
        _filterButton('Semua', 0),
        _filterButton('Sudah dibaca', 1),
        _filterButton('Belum dibaca', 2),
      ],
    ),
  );

  Widget _filterButton(String label, int index) => Expanded(
    ...
    child: Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            notification.time,
            style: const TextStyle(fontSize: 10, color: Color(0xFF8B7D75)),
          ),
        ],
      ),
    ),
